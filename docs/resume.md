...
物理サーバ・ネットワークからクラウド基盤まで、15年以上にわたり設計・構築・運用を経験してきました。AWSを約10年、Azureを約3.5年扱い、クラウド移行、IaC、CI/CD、監視設計に取り組んでいます。2022年からは自社サービスの取締役・CTOとして、要件定義から開発、運用・保守まで一貫して担当しています。

## 自己PR

### 信頼性と再現性を考慮した基盤設計

AWS・Azureの基盤設計と、IaC・CI/CDによる環境構築の標準化を得意としています。決済基盤ではTerraformで開発・検証・本番のほぼ全環境をコード化し、数時間で再構築できる状態に整備しました。自動スケールや監視・通知、稼働率目標の設計も担当し、構築後の運用を見据えて基盤を整えています。

### 運用負荷とコストの改善

リソースの使われ方と日々の運用を見直し、具体的な改善につなげてきました。公衆無線Wi-Fi基盤では、約10台のサーバに時間帯別のAuto ScalingとSpot・RIを適用し、AWSコストを3〜4割削減しました。

### 要件定義から運用・保守までの一貫した開発

自社サービスでは技術担当1名で、Webアプリケーション、SaaS連携、クラウド基盤を横断して開発しています。予約・受講履歴の連携やAIレポートなどを要件定義から運用・保守まで担当し、採用・予算・技術選定にも携わっています。定型業務の手順化とAIによる自動化を進め、当初3名で担当していた運用を1名で対応できる形に改善しました。
...
| ストレージ・DB(S3/RDS/DynamoDB/ElastiCache) | 約7年 | 使用中 | Wi-Fi 基盤・決済基盤のデータストア |
...
動画サービスの受託案件は2022/07 - 09に開発、2022/10 - 2023/07は修正・運用・保守を担当。自社サービス・業務委託と並行。2013/02 - 08は資格取得期間、2016/12 - 2017/12はカナダへ遊学。 社内ネットワークのリプレイス（2010/01 - 2013/01）は、社内インフラ業務と並行して担当しました。

## 案件経験

### オンラインヨガサービスの予約・受講基盤、AIレポート、法人向けポータルの開発・運用
...
グローバル向けのオンライン1on1ヨガ・サブスクリプションサービス（BtoC/BtoB）で技術全般を担当しています。個人向けのEC・予約・受講基盤、AIレッスンレポート、Zoom会議管理、法人向け予約ポータルを開発・運用しています。

#### 要件定義・技術選定と開発全体の担当

技術担当1名で、要件定義、技術選定、基本・詳細設計、実装、運用設計、保守・運用、障害対応まで担当しています。取締役・CTOとして、採用・予算に関する意思決定にも携わっています。

#### EC・予約・契約情報とレッスン配信の統合

Shopify・Sesami・Zoom・Recharge・Klaviyoを組み合わせ、EC・予約・契約情報とレッスン配信を連携しました。生徒・講師の現地時刻とJST・PT・UTCが関わる予約情報の整合性を考慮して設計・構築しています。

#### 予約・受講履歴のAPIと変更・キャンセル連携

Node.js・Expressで予約・契約情報の連携APIを実装しました。講師情報、予約データ、受講URL、タイムゾーンをShopify Metaobjectに保存して顧客と紐付け、予約変更・キャンセルに伴う会議情報の更新、顧客別の受講・注文・契約情報の取得、レッスン単位のコメント管理に対応しました。

#### AIレッスンレポートの生成・配信

Python・AWS Lambdaでレッスン字幕を取得し、OpenAI APIによるレポート生成、Gmailでのメール配信、Shopifyへの保存、Google Docsへの履歴蓄積までを自動化しました。2025年3月18日に正式リリースしています。

#### AIレポート処理の再試行・重複抑制

字幕生成の遅延に対応する再試行処理と、DynamoDBを使って重複処理を抑える実行ロックを実装しました。

#### 会議アカウントの自動割り当て

Python・AWS Lambdaで複数のZoomアカウントの会議予定を取得し、開始・終了時刻の重複判定で空きアカウントを選定しました。会議の作成・削除と、Google Calendar・スプレッドシートへの記録を連携しています。

#### 監視・アラーム通知の整備

CloudWatch・SNSからSlackへのアラーム通知を整備しています。

#### 法人向け予約ポータルの開発と認可・テスト設計

2026年7月からNext.js・Vercel・Supabaseで法人向け予約ポータルを新規開発しています。検証・本番の環境分離、PostgreSQL RLSによる認可、PlaywrightによるE2Eテストを整備しました。E2Eテストは本番データから隔離する構成を設計しています。

#### AIを活用した定型運用の自動化

2025年からClaude Codeを主軸に開発・運用を進め、定型オペレーションを再利用可能な手順として自動化しました。当初3名で担当していた運用を、AI活用により1名で対応できる形に改善しています。
